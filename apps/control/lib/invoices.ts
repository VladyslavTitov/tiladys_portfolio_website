...
export function parseBillingSettings(value: unknown) {
  ...
  if (!parsed.success) throw Object.assign(new Error('INVALID_BILLING_SETTINGS'), { details: parsed.error.flatten() });
...
export function parseInvoiceDraft(value: unknown) {
  ...
  if (!parsed.success) throw Object.assign(new Error('INVALID_INVOICE'), { details: parsed.error.flatten() });
...
export function assertInvoiceIssuable(invoice: { recipientName: string; recipientStreet: string; recipientPostalCode: string; recipientCity: string; issueDate: Date | null; lines: Array<{ taxTreatment: JobLineTaxTreatment }> }, settings: BillingSettings) {
  ...
  if (!invoice.recipientName || !invoice.recipientStreet || !invoice.recipientPostalCode || !invoice.recipientCity || !invoice.issueDate) throw new Error('INVOICE_DETAILS_INCOMPLETE');
...
export const invoiceInclude = { lines: { orderBy: { sortOrder: 'asc' as const } }, payments: { orderBy: { paidAt: 'asc' as const } }, customer: { select: { customerNumber: true } }, serviceJob: { select: { jobNumber: true, title: true } } };
