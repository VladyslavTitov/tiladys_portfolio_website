...
import { adminError, allocateNumber, parseServiceJob } from '@/lib/business-records';
...
export async function POST(req: NextRequest) {
  try { await assertOrigin(); const user = await requireUser(); const data = parseServiceJob(await req.json()); const row = await db.$transaction(async (tx) => { const customer = await tx.customer.findUniqueOrThrow({ where: { id: data.customerId }, select: { companyId: true } }); if (data.companyId && data.companyId !== customer.companyId) throw new Error('INVALID_SERVICE_JOB_COMPANY'); const jobNumber = await allocateNumber(tx, 'SERVICE_JOB', 'JOB'); const job = await tx.serviceJob.create({ data: { ...data, jobNumber }, include }); await tx.customerActivity.create({ data: { customerId: data.customerId, type: 'SERVICE_JOB_CREATED', summary: `Service job ${jobNumber} created`, metadata: { serviceJobId: job.id }, createdById: user.id } }); await tx.auditLog.create({ data: { userId: user.id, action: 'SERVICE_JOB_CREATE', entity: 'ServiceJob', entityId: job.id } }); return job; }); return NextResponse.json(row, { status: 201 }); }
