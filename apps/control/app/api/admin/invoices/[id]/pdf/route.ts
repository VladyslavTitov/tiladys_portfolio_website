...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (invoice.status !== 'DRAFT' && !invoice.issuedPdf) throw new Error('ISSUED_PDF_MISSING');
    const bytes = invoice.status !== 'DRAFT' ? Buffer.from(invoice.issuedPdf!) : await generateInvoicePdf(invoice);
    const filename = invoice.status === 'ISSUED' ? `${invoice.invoiceNumber}.pdf` : `draft-${invoice.id}.pdf`;
    return new NextResponse(new Uint8Array(bytes), { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `${_req.nextUrl.searchParams.get('preview') === '1' ? 'inline' : 'attachment'}; filename="${filename}"`, 'Cache-Control': 'private, no-store', 'X-Content-Type-Options': 'nosniff' } });
