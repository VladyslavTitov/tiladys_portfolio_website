...
export default async function CustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!row) notFound();
  const initial = { ...row, createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString(), archivedAt: row.archivedAt?.toISOString() ?? null, customerNotes: row.customerNotes.map((n) => ({ ...n, createdAt: n.createdAt.toISOString() })), activities: row.activities.map((a) => ({ ...a, createdAt: a.createdAt.toISOString() })), serviceJobs: row.serviceJobs.map((j) => ({ ...j, serviceDate: j.serviceDate?.toISOString() ?? null, finalPrice: j.finalPrice?.toString() ?? null })), invoices: row.invoices.map(serializeInvoice), files: row.files.map((file) => ({ ...file, createdAt: file.createdAt.toISOString() })) };
