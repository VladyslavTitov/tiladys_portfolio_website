...
export default async function ServiceJobsPage({ searchParams }: { searchParams: Promise<{ customerId?: string; jobId?: string }> }) {
  ...
  const [jobs, customers, priceItems, companies] = await Promise.all([
    db.serviceJob.findMany({ include: { invoices: { select: { id: true, invoiceNumber: true, status: true } }, lineItems: { orderBy: { sortOrder: 'asc' } }, files: { select: { id: true } } }, where: query.jobId ? { id: query.jobId } : undefined, orderBy: [{ serviceDate: 'desc' }, { createdAt: 'desc' }], take: 100 }),
  ...
  return <><span className="eyebrow">Work</span><h1>Service jobs</h1><p className="admin-intro">Plan and document customer work, time, pricing and private notes.</p><ServiceJobManager companies={companies} initialJobs={serialized} customers={customers} priceItems={priceItems} initialCustomerId={query.customerId ?? ''} initialJobId={query.jobId ?? ''} /></>;
}
