...
export default async function CompanyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><p><Link href="/dashboard/companies">← Companies</Link></p><span className="eyebrow">Company</span><h1>{company.name}</h1>
    ...
    <section className="panel"><h2>Activity history</h2>{activity.length ? <div className="timeline">{activity.map(a => <article key={a.id}><strong>{a.action.replaceAll('_', ' ')}</strong><small>{a.createdAt.toLocaleString('de-DE')}</small></article>)}</div> : <p>No activity recorded yet.</p>}</section>
  </>;
}
