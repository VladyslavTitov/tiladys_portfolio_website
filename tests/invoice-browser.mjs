...
try {
  ...
  customer = await db.customer.create({ data: { customerNumber: `C-2098-${String(suffix).slice(-4)}`, status: 'ACTIVE', firstName: 'Browser', lastName: 'Customer', email, street: 'Teststraße 2', postalCode: '45479', city: 'Mülheim an der Ruhr', country: 'DE' } });
  job = await db.serviceJob.create({ data: { jobNumber: `JOB-2098-${String(suffix).slice(-4)}`, customerId: customer.id, title: 'Browser invoice workflow', serviceDate: new Date(), lineItems: { create: { serviceName: 'Verified service', description: 'Customer-facing description', quantity: '2', unit: 'hour', agreedUnitPrice: '50', taxTreatment: 'VAT_STANDARD', taxRate: '19', subtotal: '100', taxAmount: '19', total: '119', priceConfirmed: true } } } });
  ...
  assert.ok(await connection.evaluate(`[...document.querySelectorAll('button')].some(button=>button.textContent.includes('Create invoice draft') && !button.disabled)`));
  await connection.evaluate(`[...document.querySelectorAll('button')].find(button=>button.textContent.includes('Create invoice draft')).click()`);
  ...
  assert.match(await connection.evaluate('document.body.textContent'), /DRAFT/); assert.match(await connection.evaluate('document.body.textContent'), /Verified service/);
  ...
  const issued = await connection.evaluate(`fetch('/api/admin/invoices/${invoiceId}/issue',{method:'POST'}).then(r=>r.json())`); assert.match(issued.invoiceNumber, /^INV-\d{4}-\d{4}$/); assert.equal(issued.total, '119'); assert.equal(issued.outstanding, '119');
  const payment = await connection.evaluate(`fetch('/api/admin/invoices/${invoiceId}/payments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({amount:'19',paidAt:new Date().toISOString(),method:'BANK',reference:'TEST',note:''})}).then(r=>r.json())`); assert.equal(payment.paidTotal, '19'); assert.equal(payment.outstanding, '100');
  await connection.navigate(`${control}/dashboard/invoices?invoiceId=${invoiceId}`); assert.match(await connection.evaluate('document.body.textContent'), /Outstanding/); assert.match(await connection.evaluate('document.body.textContent'), /100,00/);
