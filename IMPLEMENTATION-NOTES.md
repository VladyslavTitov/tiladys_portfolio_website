...
## About section order correction

The current About order is business introduction, founder/portrait, languages and education, then the Services/Contact invitation. Founder labels and the IT/web invitation are localized in all six locales; verified education and language entries are unchanged. The requested portrait URL is again `/about/vladyslav-titov.png` (file: `apps/web/public/about/vladyslav-titov.png`), superseding the JPEG setting above. A JPEG currently exists in the folder and was left untouched; the clean fallback remains until the PNG is supplied. Section spacing follows the new order.
